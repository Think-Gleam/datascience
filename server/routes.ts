...
export async function registerRoutes(
  ...
  app.get("/api/courses", async (_req, res, next) => {
    try {
      const publishedOnly =
        !_req.isAuthenticated() || _req.user?.role !== "admin";
  ...
  app.get("/api/courses/:id", async (req, res, next) => {
    try {
      ...
      const allLessons = await storage.getLessonsByModules(
  ...
  app.get(
    ...
    async (req, res, next) => {
      try {
        ...
        const completedLessons = await storage.getLessonProgressByCourse(
          req.user!.id,
          courseId,
        );
        ...
        res.json({
          ...
          percentage:
            totalLessons.length > 0
              ? Math.round((completedCount / totalLessons.length) * 100)
              : 0,
        });
      } catch (err) {
        next(err);
  ...
  app.get(
    "/api/enrollments/:courseId/lesson-progress",
    requireAuth,
    async (req, res, next) => {
      try {
        const courseId = Number(req.params.courseId);
        ...
        if (!enrollment)
          return res.status(404).json({ message: "Not enrolled" });
        const progress = await storage.getLessonProgressByCourse(
          req.user!.id,
          courseId,
        );
        res.json(progress);
  ...
  app.get("/api/admin/stats", requireAdmin, async (req, res, next) => {
    try {
      const [userCount, courseCount, enrollmentCount, certificateCount] =
        await Promise.all([
